...
for(let type in optionTypes){
  describe(`Component OrderOption with type=${type}`, () => {
    ...
    switch (type) {
      ...
      case 'icons': {
        it('contains component and icon classNames', () => {
          const mainComponent = renderedSubcomponent.find('.component');
          expect(mainComponent.length).toBe(1);

          const iconClass = renderedSubcomponent.find('.icon');
          expect(iconClass.length).toBe(2);

          const emptyOption = renderedSubcomponent.find('Icon[name="times-circle"]');
      ...
      case 'checkboxes': {
        it('contains label and input', () => {
          ...
          const inputType = renderedSubcomponent.find('input[type="checkbox"]');
          expect(inputType.length).toBe(2);
        });
        it('should run setOrderOption function on change', () => {
